...
public final class ReplaceBits {
	...
	public static int replace(int q, int p, int i, int j) {
		...
		if(q < 0 || p < 0) {
			throw new IllegalArgumentException("The q aND P number must be positive");
		}
		
		if(i < 0 || j < 0 || i > 31 || j > 31 || i >= j) {
			throw new IllegalArgumentException("The i and j values don't follow th problem instructions");
		}
